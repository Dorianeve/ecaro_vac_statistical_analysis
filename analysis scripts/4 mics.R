...
mics <- icvac %>% filter(Indicator == "Violent discipline - Any psychological aggression")
...
output_folder <- "analysis/cat4_MICS/"
...
for (country in unique_countries) {
  ...
  for (col in filter_columns) {
    ...
    plot <- ggplot(filtered_data, aes(x = .data[[col]], y = Value, fill = .data[[col]])) +
      ...
      labs(
        title = paste("Violent discipline - Any psychological aggression for", country),
...
p <- static_map(data)
ggsave(filename = "analysis/cat4_MICS/static_map_with_auto_zoom_adapted_frame.png",
